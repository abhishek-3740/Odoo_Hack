...
class ArchitectureFlow(Flowable):
    ...
    def draw(self):
        c=self.canv
        for x, title, detail in [(0,'Internal workspace','Rep / Manager / Finance'),(168,'Customer portal','Own quotations only'),(336,'Scheduled jobs','Billing / time-based alerts')]:
            box(c,x,111,154,43,title,detail,size=9)
            draw_arrow(c,x+77,111,x+77,92)
        box(c,0,50,490,42,'Verified API + shared domain services','Pricing • Approval • Recommendations • Allocation • Billing',dark=True,size=10)
        draw_arrow(c,245,50,245,35)
        box(c,65,0,360,34,'PostgreSQL — snapshots, transactions, audit trail',size=9)
...
class Doc(BaseDocTemplate):
    ...
    def decorate(self,c,doc):
        ...
        c.drawString(43,19,'Planning edition • 05 September 2026 • 4 members / 24 hours')
...
def architecture_page():
    ...
    c.setFillColor(MUTED); c.setFont('Body',9.4); c.drawString(32,h-62,'One Next.js application • Supabase Auth + PostgreSQL • Deterministic engines • Proposed build architecture')
    xs=[32,232,432,632]
    specs=[('Internal workspace','Rep / Manager / Finance / Admin'),('Customer portal','Own quotations and terms'),('Supabase Auth','Verified identity + stored roles'),('Supabase Cron','Authenticated, retry-safe jobs')]
    for x,(title,detail) in zip(xs,specs):
        box(c,x,438,178,54,title,detail,size=9.7)
        draw_arrow(c,x+89,438,x+89,420)
    box(c,32,300,778,120,'NEXT.JS API + DOMAIN SERVICES','Authorization and safe DTOs • Quote version gates • Shared transaction context',True,size=12)
    ...
    c.setFillColor(MUTED); c.setFont('Body',8.2); c.drawString(32,248,'Arrows show ownership/relationships. Audit events and transaction constraints span all commercial records.')
    ...
    low=[('Catalog + plans','Variants / prices / cadence'),('Quote lines','Quantity / price / discount'),('Approval decisions','Version / actor / reason'),('Billing records','Subscriptions / invoices\nPayments / credits / refunds'),('Inventory','Stock rows / movements')]
    ...
    c.setFillColor(MUTED); c.setFont('Body',7.5); c.drawString(32,17,'Planning edition • 05 September 2026 • Full fields and tests are in the Implementation Plan')
    c.save()

if __name__=='__main__':
    architecture_page()
    render_markdown(ROOT/'DealFlow360-Solution-Report.md','Solution Report',
        'A product and architecture proposal for a sales platform that connects quotation, negotiation, approval, fulfillment, and billing.',
        [('Core decision','One full-stack TypeScript application and PostgreSQL.'),('Machine learning','No trained model required for the stated requirements.'),('Creative extension','Deal Lab: explain valid alternatives before changing a live deal.'),('Read with','The companion implementation plan and one-page architecture diagram.')])
    render_markdown(ROOT/'DealFlow360-Implementation-Plan.md','Implementation Plan',
        'An execution specification covering frameworks, data, formulas, APIs, permissions, four-person ownership, tests, deployment, and the live demonstration.',
        [('Delivery rule','Required workflows first; optional innovation has a bounded slot.'),('Engineering focus','Immutable quote versions, atomic stock reservation, retry-safe billing.'),('Team coordination','Four vertical workstreams with shared contracts and integration gates.'),('Verification','28 acceptance checks, two complete journeys, explicit numeric fixtures.')])
